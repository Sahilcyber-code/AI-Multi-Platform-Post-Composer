...
function PreviewCard({ platformId, post, mediaFile }) {
  ...
  const renderMediaPreview = () => {
    ...
    const isImage =
      ...
      /\.(jpeg|jpg|png|gif)$/i.test(mediaFile.path);

    const isVideo =
      mediaFile.type?.startsWith('video/') ||
      /\.(mp4|mov|webm)$/i.test(mediaFile.path);

    // Works both locally (through Vite proxy) and on Render
  ...
  return (
    <section
      ...
      style={{
        borderTop: `4px solid ${rule?.color || '#3b82f6'}`,
      }}
    >
